 ...
 * @LastEditTime: 2022-06-13 12:38:01
 */
    ...
    methods: {
        ...
        getExcel_city(res) {
            require.ensure([], () => {
                ...
                var filename = "候选人信息-" + this.$dayjs(new Date()).format("YYYYMMDDHHmmss");
        ...
        handleImportData(item, originData) {
            stageMap.forEach((stage) => {
                const status = stage.status.find((ite) => ite.name === item.statusName);
                if (status) {
            ...
            apartments.forEach((apartment) => {
                const job = apartment.jobs.find((ite) => ite.name === item.jobName);
                if (job) {
                    item.jobId = job.id;
            ...
            item.typeId = configs.type.find((ite) => ite.name === item.typeName)?.id || "";
            item.schoolPropertyId =
                configs.schoolProperty.find((ite) => ite.name === item.schoolPropertyName)?.id || "";
            item.degreeId = configs.degree.find((ite) => ite.name === item.degreeName)?.id || "";
            item.channelId = channelMap[item.channelName] || "";
            if (originData["面试官"]) {
                // 一面信息
                const obj = {
                    interviewDate: new Date(originData["面试日期"]).toISOString(),
                    interviewTime: originData["面试时间"],
                    interviewerId: originData["面试官"].split(",").map((ite) => {
                        return interviewers.find((interviewer) => interviewer.name === ite).id;
                    }),
                    interviewerName: originData["面试官"].split(","),
                    modeId: configs.mode.find((ite) => ite.name === originData["面试形式"])?.id || 0,
            ...
            if (originData["面试官_1"]) {
                // 一面信息
                const obj = {
                    interviewDate: new Date(originData["面试日期_1"]).toISOString(),
                    interviewTime: originData["面试时间_1"],
                    interviewerId: originData["面试官_1"].split(",").map((ite) => {
                        return interviewers.find((interviewer) => interviewer.name === ite).id;
                    }),
                    interviewerName: originData["面试官_1"].split(","),
                    modeId: configs.mode.find((ite) => ite.name === originData["面试形式_1"])?.id || 0,
            ...
            if (originData["面试官_2"]) {
                // 一面信息
                const obj = {
                    interviewDate: new Date(originData["面试日期_2"]).toISOString(),
                    interviewTime: originData["面试时间_2"],
                    interviewerId: originData["面试官_2"].split(",").map((ite) => {
                        return interviewers.find((interviewer) => interviewer.name === ite).id;
                    }),
                    interviewerName: originData["面试官_2"].split(","),
                    modeId: configs.mode.find((ite) => ite.name === originData["面试形式_2"])?.id || 0,
